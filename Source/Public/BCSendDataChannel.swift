...
public class BCSendDataChannel: NSObject {
    ...
    public var hasDataTasksNotSent: Bool {
        lock.lock()
    ...
    public init(peripheral: CBPeripheral, characteristic: CBCharacteristic) {
    ...
    public func peripheralIsReadyToSendData() {
        processSendDataTasks()
    }
    
    /// 取消数据通道中的所有数据发送任务，比如蓝牙断开
    public func cancelAllSendDataTasks() {
        lock.lock()
